...
# Build with the embedded sandbox-init helper (requires: make build-sandbox-init first).
build-with-sandbox:
	GOOS=linux go build -tags sandbox_init -ldflags '$(PLUGIN_LDFLAGS)' -o $(BINARY_NAME) ./cmd/plugin-gitops
